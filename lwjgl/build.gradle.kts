...
tasks.jar {
    this.archiveBaseName.set("graphics-lwjgl")
}
...
publishing {
    publications {
        create<MavenPublication>("lwjgl-maven") {
            from(components["java"])
            artifact(tasks["sourcesJar"])
            artifact(tasks["javadocJar"])

            artifactId = "graphics-lwjgl"

            pom {
                name.set("Graphics LWJGL")
                description.set("Implementation for the YakClient graphics library for LWJGL")
...
//signing {
//    sign(publishing.publications["lwjgl-maven"])
//}